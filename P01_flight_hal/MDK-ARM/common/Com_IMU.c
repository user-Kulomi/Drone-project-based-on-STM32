                              ...
                              float dt)
{
    ...
    // 加速度归一化
    NormQuat = Q_rsqrt(squa(gyroAccel->acc_data.accel_x) +
                       squa(gyroAccel->acc_data.accel_y) +
                       squa(gyroAccel->acc_data.accel_z));

    Acc.x = gyroAccel->acc_data.accel_x * NormQuat;
    Acc.y = gyroAccel->acc_data.accel_y * NormQuat;
    Acc.z = gyroAccel->acc_data.accel_z * NormQuat;
    // 向量差乘得出的值
    ...
    // 角速度融合加速度积分补偿值
    Gyro.x = gyroAccel->gyro_data.gyro_x * Gyro_Gr + KpDef * AccGravity.x + GyroIntegError.x;   // 弧度制
    ...
    float yaw_G = gyroAccel->gyro_data.gyro_z * Gyro_G;   // 将Z轴角速度陀螺仪值 转换为Z角度/秒      Gyro_G陀螺仪初始化量程+-2000度每秒于1 / (65536 / 4000) = 0.03051756*2
    ...
    normAccz = gyroAccel->acc_data.accel_x * vecxZ + gyroAccel->acc_data.accel_y * vecyZ + gyroAccel->acc_data.accel_z * veczZ; /*Z轴垂直方向上的加速度，此值涵盖了倾斜时在Z轴角速度的向量和，不是单纯重力感应得出的值*/
}
/**
